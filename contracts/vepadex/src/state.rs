    ...
    pub fn locked_amount_at_timestamp(&self, timestamp: u64) -> Uint128 {
        ...
        Uint128::try_from(
            Uint256::from(self.deposited_amount)
                - Uint256::from(self.deposited_amount)
                    * Uint256::from(self.elapsed_lock_time_at_timestamp(timestamp))
                    / Uint256::from(self.initial_lock_duration()),
        )
        .unwrap()
    }
...
impl QuadraticEquationCoefficients {
    pub fn evaluate_voting_power_at_timestamp(&self, timestamp: u64) -> Uint128 {
        Uint128::try_from(
            ((
                // Floor
                self.constant_coefficient.numerator() / self.constant_coefficient.denominator()
                // Floor
                + Uint256::from(timestamp) * Uint256::from(timestamp) * self.quad_coefficient.numerator()
                / self.quad_coefficient.denominator())
                // Subtracts a truncated value value
                .checked_sub(Uint256::from(timestamp) * self.linear_coefficient.numerator() / self.linear_coefficient.denominator()))
    ...
    pub fn evaluate_locked_balance_at_timestamp(&self, timestamp: u64) -> Uint128 {
        Uint128::try_from(
            // Floor
            (self.linear_coefficient.numerator()
                / self.linear_coefficient.denominator()
                / Uint256::from(2u128))
            // Subtracts a truncated value value
            .checked_sub(
                Uint256::from(timestamp) * self.quad_coefficient.numerator()
                    / self.quad_coefficient.denominator(),
            )
            // In the event of an underflow
            // which can happen because of truncation
